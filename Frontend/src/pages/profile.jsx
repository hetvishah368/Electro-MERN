...
const Profile = () => {
  ...
  const containerStyle = { maxWidth: '1000px', margin: '40px auto', padding: '30px', background: '#1e293b', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', color: '#fafafa' };
  ...
  return (
    <div style={containerStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '30px', marginBottom: '30px' }}>
        ...
        <button onClick={handleLogout} className="btn" style={{ background: '#ef4444', boxShadow: 'none', width: '120px' }}>Logout</button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#0f172a', padding: '30px', borderRadius: '8px', textAlign: 'center', border: '1px solid #27272a' }}>
        ...
        <div style={{ display: 'grid', gap: '20px' }}>
          {orders.map(order => (
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
              ...
              <div>
                <span style={{ 
                  background: order.paymentStatus === 'delivered' ? 'rgba(16,185,129,0.1)' : order.paymentStatus === 'shipped' ? 'rgba(59,130,246,0.1)' : 'rgba(245,158,11,0.1)', 
                  color: order.paymentStatus === 'delivered' ? '#10b981' : order.paymentStatus === 'shipped' ? '#3b82f6' : '#f59e0b',
